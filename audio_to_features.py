"""
Script for collecting wav files from a directory and converting them to features.

The script performs the following tasks:
- Collects wav files from a specified directory.
- Filters the collected files based on specific criteria.
- Converts the filtered audio files to feature representations.
- Saves the features in a designated destination directory.

The script relies on various utility functions and modules for file retrieval, filtering, and feature extraction.
...
def nus_filter(root_dir, config):
    """Filter function for NUS dataset."""
    config.audio_ext = "wav"

    _, file_list = recursive_file_retrieval(root_dir)
    file_list = [
        fp
        for fp in file_list
        if fp.endswith(config.audio_ext) and not fp.startswith(".")
    ]
    return file_list


def vctk_filter(root_dir, config):
    """Filter function for VCTK dataset."""
    ...
    filtered_list = [fp for fp in file_list if fp[: -len(config.audio_ext)][-1] != "2"]
    return filtered_list


def damp_filter(root_dir, config):
    """Filter function for DAMP dataset."""
...
def wav_filter(root_dir, config):
    """Filter function for WAV files."""
    config.audio_ext = ".wav"
    _, file_list = recursive_file_retrieval(root_dir)
    filtered_list = [
        fp
...
def vocalset_filter(root_dir, config):
    """Filter function for VocalSet dataset."""
    config.audio_ext = ".wav"
    # Declare variables
    balance_list = True
    use_subclass = True
    ...
    if use_subclass:
        ...
        exclusive_list = substring_exclusion(file_list, exclude_list)
        inclusive_list = substring_inclusion(exclusive_list, class_list)
        file_list = inclusive_list

    filtered_list = [
        f for f in file_list if f[-6] == "_"
    ]  # To ensure that the second last char of the filename is '_', as we are not interested in deviating formats
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Params for converting audio to spectral using world",
    ...
    if config.find_diff:
        ...
        dst_dir_all_fns = [
            os.path.basename(path)[: -len(config.dst_ext)] for path in dst_dir_all_fps
        ]
        filtered_list = [
            path
            for path in filtered_list
            if os.path.basename(path)[: -len(config.audio_ext)] not in dst_dir_all_fns
        ]
    ...
    if config.feat_type == "mel":
        mel_filter = mel(
            ...
            n_mels=feat_params["num_harm_feats"],
        ).T
        ...
        raise Exception("feat_type param was not recognized.")

...
SieEncoderTrainer is an object that initiates a model based on user inputs
...
class SieEncoderTrainer:
    """Create encoder model and digest data from given datasets with specific training schedule
    
    """

    def __init__(self, config, feat_params=None) -> None:
        """Initialilize the SieEncoderTrainer object

        Create encoder model and datasets with given configurations.
        Train the model either from a checkpoint or from scratch
        Iterate between training and validation cycles
        Print progress
        ...
        self.config.midi_range = range(
            36, 82
        )  # range between Db5 and Ab5 (1 semitone wider than the World wav2world default params)

        # Load feature parameters from dataset yaml
        if feat_params == None:
            with open(
                os.path.join(self.config.feature_dir, "feat_params.yaml")
            ) as File:
        ...
        if not self.config.eval_only:
            self.train_dataset = SpeakerVerificationDataset(
                config.feature_dir.joinpath("train"),
                config,
                feat_params,
        ...
        if config.norm_method == "schluter" or config.norm_method == "global_unit_var":
            ...
            self.val_dataset = SpeakerVerificationDataset(
                config.feature_dir.joinpath("val"),
                config,
                feat_params,
                self.num_total_feats,
                norm_stats,
        ...
        if config.pitch_condition:
            self.num_total_feats = (
                self.num_total_feats + len(self.config.midi_range) + 1
            )
        ...
        os.makedirs(
            os.path.join(self.this_model_dir, "input_tensor_plots"), exist_ok=True
        )

        self.backprop_losses = {"ge2e": 0.0, "pred": 0.0, "both": 0.0, "acc": 0.0}
        self.print_iter_metrics = {"ge2e": 0.0, "pred": 0.0, "both": 0.0, "acc": 0.0}
        self.entire_iter_metrics = {"ge2e": 0.0, "pred": 0.0, "both": 0.0, "acc": 0.0}
        ...
        self.EarlyStopping = EarlyStopping(
            patience=config.patience, threshold=config.earlystop_thresh
        )
    ...
    def get_iter_size(self, config, subset):
        """Calculate the appropriate number of iterations for the given dataset
        and batch size, and utterances per speaker

        Args:
            config (argparaser): Argparser object with configurations.
            subset (str): Name of the subset for which iteration number is calculated
        ...
        train_iters = int(
            (num_train_subdirs // config.speakers_per_batch)
            * ((avg_uttrs_per_spkr * chunks_per_track) // config.utterances_per_speaker)
        )
        return train_iters
    ...
    def train(self):
        """
        Advance the training of the model through a scheduled iteration system
        cycling between training and validation phases.
        """
        # self.tester()
    ...
    def batch_iterate(self, loader, initial_iter_step, mode):
        """
        Execute operations for scheduled training, based on which phase (mode) is active.

        Args:
            loader (dataloader): Custom dataset's loader (either training or val)
            intial_iter_step (int): Signify what training step we are currently on
            mode (str): Inform method which phase of scheduled training its in - train or val
        """
        ...
        for step, speaker_batch in enumerate(loader, initial_iter_step + 1):
            ...
            finish_iters = (
                step != 0 and step % (initial_iter_step + self.mode_iters[mode]) == 0
            )
            ...
            if step < 5:
                ...
                ex_path = os.path.join(
                    self.this_model_dir, "input_tensor_plots", f"step {step}"
                )
                array_to_img(np.rot90(x_data_npy[0]), ex_path)

            # Forward pass
            inputs = (
                torch.from_numpy(x_data_npy).to(self.device).float()
            )  # speakerbatch shape = speakers, timesteps, features
            ...
            for i, key in enumerate(self.backprop_losses.keys()):
                self.backprop_losses[key] = metrics_list[i]
            for i, key in enumerate(self.print_iter_metrics.keys()):
                self.print_iter_metrics[key] += metrics_list[i]
            for i, key in enumerate(self.entire_iter_metrics.keys()):
                self.entire_iter_metrics[key] += metrics_list[i]
            ...
            if finish_iters:
                ...
                if mode == "val":
                    ...
                    if check == "lowest_loss":
                        ...
                        print(
                            f"Saved model (step {self.train_current_step}) at time {time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(time.time()))}"
                        )
                    if check == 0:
    ...
    def get_avg_metrics(self, step, mode):
        """
        Print metrics of model performance to console and save to tensorboard

        Args:
            step (int): the current training step
            mode: the phase in the scheduled training cycle
        """
        # print average metrics
        print(f"Mode: {mode}: AVERAGE PER ITER BLOCK")
        accuracy = round(self.entire_iter_metrics["acc"] / self.mode_iters[mode], 4)
        ge2e_loss = round(
            self.entire_iter_metrics["ge2e"].item() / self.mode_iters[mode], 4
        )
        pred_loss = round(
            self.entire_iter_metrics["pred"].item() / self.mode_iters[mode], 4
        )
        both_loss = round(
            self.entire_iter_metrics["both"].item() / self.mode_iters[mode], 4
        )
        ...
        print(
            f"Steps {this_step}/{self.config.stop_at_step}, Accuracy: {accuracy}, GE2E loss: {ge2e_loss}, Pred loss: {pred_loss}, Total loss: {round(both_loss, 4)} \n"
        )
    ...
    def config_model(self):
        """
        Build a new AutoVC model using configurations, or continue from a previously saved version.
        """

        new_model_name_error = "Your run_id name already exists. If this refers to the model you want to continue training from, you must define the new save location using the new_run_id parameter. Do this by including including \'-rid [NEW_MODEL_NAME]\' when running \'python main.py\'"
        ...
        if self.config.run_id == "testRuns":
            ...
            model = SpeakerEncoder(
                self.device,
                self.loss_device,
                self.val_dataset.num_voices()
                if self.config.eval_only
                else self.train_dataset.num_voices(),
                self.num_total_feats,
            ...
            optimizer = torch.optim.Adam(
                model.parameters(), lr=self.config.learning_rate_init
            )
            ...
            if os.path.exists(run_id_path):  # if the run_id model exist
                if self.config.new_run_id == None:  # and new_run_dir is defined
                    ...
                    writer = SummaryWriter(
                        comment="_" + os.path.basename(self.config.new_run_id)
                    )
                    new_save_dir = os.path.join(run_id_path, self.config.new_run_id)
                    overwrite_dir(new_save_dir, self.config.ask)
                    open(new_save_dir + "/config.txt", "w").write(
                        self.config.string_sum
                    )
    ...
    def get_losses(self, embeds, predictions, y_data):
        """
        Calculate GE2E loss and prediction loss

        Args:
            embeds (array): output of SIE encoder
            predictions (array): predictions of singer ID based on embeds
            y_data (array): ground truth numeric labels for singer ID
        """

        embeds_loss = embeds.view(
            (self.config.speakers_per_batch, self.config.utterances_per_speaker, -1)
        ).to(
            self.loss_device
    ...
    def backprop_ops(self, mode):
        """
        Execute necessary operations to initiate backpropogation

        Args:
            mode (str): indicates which phase of scheduled training cycle is active
        """
        if mode == "train":
            self.backprop_losses[
                self.config.use_loss
            ].backward()  # if loss was generated without using parameters adjusted in do_gradient_ops(), there will be no gradient after backwards upon which to adjust
    ...
    def save_by_val_loss(self, current_loss, name="saved_model.pt"):
        ...
        Args:
            current_loss (int): The current loss from the SIE encoder's output
            name (str): desired name of saved file
        """
    ...
    def print_monitor(self, step, mode):
        """
        Print metrics used to train encoder model, reset their values when done
        ...
        if mode == "train":
            print(
                f"Elapsed: [{et}], Mode: {mode}, Steps {step}/{self.config.stop_at_step}, Accuracy: {round(pred_acc, 4)}, GE2E loss: {round(ge2e_loss, 4)}, Pred loss: {round(pred_loss, 4)}, Total loss: {round(total_loss, 4)}"
            )
        else:
            print(
                f"Elapsed: [{et}], Mode: {mode}, Steps {self.train_current_step}/{self.config.stop_at_step}, Accuracy: {round(pred_acc, 4)}, GE2E loss: {round(ge2e_loss, 4)}, Pred loss: {round(pred_loss, 4)}, Total loss: {round(total_loss, 4)}"
            )
        for key in self.print_iter_metrics.keys():
            self.print_iter_metrics[key] = 0
